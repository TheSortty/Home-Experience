...
export async function GET(request: Request) {
  ...
  try {
    ...
    // supabase-js NO tira excepción ante un error de Postgres: devuelve { error }.
    // Sin mirarlo, un fallo de permisos acá pasaba desapercibido y el alumno
    // volvía con "conectado" aunque no se hubiera guardado nada.
    const { error: saveError } = await db.from('google_calendar_accounts').upsert({
    ...
    if (saveError) {
      console.error('[gcal] no se pudo guardar la cuenta:', saveError);
      return back('error');
    }
